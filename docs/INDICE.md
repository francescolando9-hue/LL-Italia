...
|---|---|
| `AppBolleSpecificaFunzionale202609011935Claude.md` | **Specifica funzionale, rev. 5 — prevale su tutto** per il modulo Bolle: contratto di invio (token nel corpo, `dataInvio`, `202 Accepted` senza corpo, `api-version=2024-10-01`), campi `progressivo`, `idDispositivo`, **`fase`** (dalla 0.31.0) e i tre **livelli `piano` / `unita` / `prospetto`** (dalla 0.36.0: **quattro colonne da aggiungere** in `BolleInArrivo` — `Fase`, `Piano`, `Unita`, `Prospetto`), il **lotto al posto della fase** per SNU e BRU, **«Rimanda»** su ogni bolla inviata, barra dei comandi fissa per gli scatti in serie. |
