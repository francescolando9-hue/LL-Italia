...
- **Colonna `Fase` (riga di testo singola): DA AGGIUNGERE con la 0.31.0** e mappare in *Update file properties* sul campo `fase` del payload. Valori: solo quelli dell'elenco chiuso (`core/fasi.js`), grafia esatta. Vuota = bolla di una versione precedente alla 0.31.0. Finché la colonna non c'è il flow ignora il campo e le bolle atterrano lo stesso. Due voci dell'elenco (`CMC 128`, `Impianto SEFCC`) sono da verificare con Francesco: se cadono, vanno tolte dall'app **prima** che qualcuno le usi, non dopo.
