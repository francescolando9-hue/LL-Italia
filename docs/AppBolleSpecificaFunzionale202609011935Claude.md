...
> **Revisione 5 del 18/09/2026** — tre cose, tutte dalla **0.36.0**, decise da Francesco il 18/09/2026. (1) **Tre campi nuovi nel contratto: `piano`, `unita`, `prospetto`.** Il selettore della fase è **condiviso col modulo Foto**, e i livelli stanno attaccati alla fase: se la fase scelta pretende un piano o un'unità, il campo compare ed è obbligatorio anche qui. Codici senza spazi, `null` dove non si applicano, **mai stringa vuota**. **Servono tre colonne nuove in `BolleInArrivo`** (vedi *Ricezione*): finché non ci sono, il flow ignora i campi e le bolle atterrano lo stesso. (2) **Per le urbanizzazioni (`SNU`, `BRU`) il LOTTO prende il posto della fase**, e viaggia nello stesso campo `fase` col suo codice (`Lotto2`): anche una bolla di SNU prende il lotto, è voluto. (3) **«Rimanda» su ogni bolla inviata**, non più solo dallo storico e non più solo per correggere il cantiere: senza modifiche riusa lo stesso `idClient` e il flow risponde `gia_presente`; con modifiche è un invio nuovo con `idClient` e progressivo nuovi, e **l'`idBolla` resta quello dell'originale** perché una pagina corretta è la stessa pagina della stessa bolla. La regola dei livelli e l'anagrafica di piani, unità, prospetti e lotti stanno nella specifica del modulo Foto, §4.5, e in `core/anagrafica.js`: **una sola verità per i due moduli.**
>
...
{ "token": "collaudo",
  ...
  "piano": "P1",                // livelli dell'archivio (rev. 5), codici senza spazi.
  "unita": "1A",                //   null dove il livello non si applica,
  "prospetto": null,            //   MAI stringa vuota. Regola: specifica Foto §4.5
...
- **Colonne `Piano`, `Unita`, `Prospetto` (riga di testo singola): DA AGGIUNGERE con la 0.36.0** e mappare in *Update file properties* sui campi `piano`, `unita`, `prospetto` del payload. Nomi senza accento e senza spazi, come le altre. Contengono i **codici** così come arrivano — `P1`, `P-2`, `1.01` col punto, `Nord` — e **niente conversioni**: quello che arriva in colonna è anche il nome della cartella sul server. Vuote dove la fase non prevede quel livello: è il loro significato. Le regole di quando un livello è obbligatorio stanno nella specifica del modulo Foto, §4.5 — **non si duplicano qui**, perché il selettore è lo stesso e due copie divergerebbero.
