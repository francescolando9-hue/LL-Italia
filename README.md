...
1. **Obiettivo grandangolare nella fotocamera in-app.** Scattando dall'app il telefono apre la fotocamera **ultra-grandangolare**: inquadra tutto il foglio, ma distorce. Si vuole l'obiettivo **normale**. Oggi l'app chiede solo `facingMode: environment` e lascia scegliere al browser (`core/fotocamera.js`). Da chiudere: su quali telefoni succede (modello e browser), e se si vuole un pulsante per cambiare obiettivo oltre alla scelta automatica.
2. **Disposizione dei comandi nella fotocamera in-app.** Il pulsante verde *Fine* è enorme e quello di scatto sta in basso a sinistra: scomodo. Si vuole lo **scatto al centro**, dove ci si aspetta di trovarlo, e *Fine* **alla sua sinistra**. Vale per entrambi i moduli, perché la fotocamera è una sola. Da chiudere: cosa sta a destra dello scatto (lo spazio si presta al cambio obiettivo del punto 1).
